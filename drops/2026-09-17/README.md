# Aurell — Tideglass Skimmer still

Free mystical pet still from **Minzy Studios**.

**Aurell** is an original Tideglass Skimmer: dusk-violet otter-ray silhouette, translucent seaglass fins, soft teal bioluminescence. A calm moonlit tide-pool companion still for teens and young adults — not a product mascot, just a free giveaway image.

## What's in this drop

| File | What it is |
| --- | --- |
| `art/aurell-tideglass-still.png` | Square mystical pet still (social / phone) |
| `art/aurell-tideglass-desktop.png` | Wide desktop / lock-screen wallpaper |
| `captions.md` | Social caption drafts (Mike owns publish) |

Mirrored under [`desktop-art/aurell/`](../../desktop-art/aurell/).

## How to use

1. Download the square still for phone wallpaper / social share, or the wide file for desktop.
2. Set as wallpaper or keep as a calm focus image.
3. Personal use. Credit Minzy Studios appreciated.

## License / use

- **Art:** Free for **personal** use. Credit **Minzy Studios** appreciated — [https://minzystudios.com](https://minzystudios.com). Also offered under **CC BY 4.0** unless noted otherwise.
- Commercial / redistribution beyond personal use: ask hello@minzystudios.com.

## Credit

- Studio: [Minzy Studios](https://minzystudios.com)
- Contact: hello@minzystudios.com
- Art: Grok Imagine (AETHERION free-drop engine), 2026-09-17

## Download

Repo: [github.com/mikebelyea/minzystudios-freebies](https://github.com/mikebelyea/minzystudios-freebies)

This drop: `drops/2026-09-17/`
