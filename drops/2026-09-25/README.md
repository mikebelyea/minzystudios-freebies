# Sylrix Mossglass - free desktop wallpapers

Free epic wallpapers from **Minzy Studios**.

**Sylrix Mossglass** is an original mystical moss-glass skink with translucent emerald and amber scales and soft moss along the spine. Quiet meadow dawn energy. Calm wildlife wallpaper mood. Not a product mascot. Just a free giveaway set.

Body plan: reptile (skink / small lizard). Clear rotation away from fox, otter, mink, owl, and moth silhouettes.

## What's in this drop

| File | What it is |
| --- | --- |
| `art/sylrix-mossglass-stone-perch.png` | Desktop wallpaper, skink on sunwarmed stone at meadow dawn (**5120x2880**) |
| `art/sylrix-mossglass-fern-crawl.png` | Desktop wallpaper, skink crawling through ferns and dew (**5120x2880**) |
| `art/*.jpg` | Same two desktops as high-quality JPEG |
| `art/sylrix-mossglass-portrait-still.png` | Square mystical still for phone / social (**2048x2048**) |
| `art/*-still.png` | Square stills (**2048x2048**) center-cropped from each desktop |
| `art/*-phone.png` | Portrait phone crops (**1440x2560**) |
| `captions.md` | Social caption drafts (Mike owns publish) |

## How to use

1. Download a wide PNG or JPG for desktop / lock-screen wallpaper.
2. Use a still or phone crop for mobile or social share.
3. Personal use. Credit Minzy Studios appreciated.

## License / use

- **Art:** Free for **personal** use. Credit **Minzy Studios** appreciated - [https://minzystudios.com](https://minzystudios.com). Also offered under **CC BY 4.0** unless noted otherwise.
- Commercial / redistribution beyond personal use: ask hello@minzystudios.com.

## Credit

- Studio: [Minzy Studios](https://minzystudios.com)
- Contact: hello@minzystudios.com
- Creature: Sylrix Mossglass (mystical moss-glass skink, meadow dawn)
- Art: Flux schnell (AETHERION free-drop engine), RealESRGAN_x4plus HQ, 2026-09-25

## Download

Repo: [github.com/mikebelyea/minzystudios-freebies](https://github.com/mikebelyea/minzystudios-freebies)

This drop: `drops/2026-09-25/`
